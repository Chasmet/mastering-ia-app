...
function normalizeStemPlan(plan) {
  ...
  return {
    ...
    voicePlan: {
      ...
      settings:
        voicePlan.settings && typeof voicePlan.settings === "object"
          ? {
              ...
              compression_attack_ms: clamp(
                safeNumber(
                  voicePlan.settings.compression_attack_ms,
                  fallback.voicePlan.settings.compression_attack_ms
                ),
                1,
                200
              ),
              compression_release_ms: clamp(
                safeNumber(
                  voicePlan.settings.compression_release_ms,
                  fallback.voicePlan.settings.compression_release_ms
                ),
                20,
                1000
              )
            }
          : fallback.voicePlan.settings
    },
    instrumentalPlan: {
      goal:
      ...
      settings:
        instrumentalPlan.settings && typeof instrumentalPlan.settings === "object"
          ? {
              low_gain_db: clamp(
                safeNumber(
                  instrumentalPlan.settings.low_gain_db,
                  fallback.instrumentalPlan.settings.low_gain_db
                ),
                -6,
                6
              ),
              presence_gain_db: clamp(
                safeNumber(
                  instrumentalPlan.settings.presence_gain_db,
                  fallback.instrumentalPlan.settings.presence_gain_db
                ),
                -6,
                6
              ),
              glue_ratio: clamp(
                safeNumber(
                  instrumentalPlan.settings.glue_ratio,
                  fallback.instrumentalPlan.settings.glue_ratio
                ),
                1.1,
                6
              ),
              glue_attack_ms: clamp(
                safeNumber(
                  instrumentalPlan.settings.glue_attack_ms,
                  fallback.instrumentalPlan.settings.glue_attack_ms
                ),
                1,
                200
              ),
              glue_release_ms: clamp(
                safeNumber(
                  instrumentalPlan.settings.glue_release_ms,
                  fallback.instrumentalPlan.settings.glue_release_ms
                ),
                20,
                1200
              )
            }
          : fallback.instrumentalPlan.settings
    },
    recombinationPlan: {
      voiceLevelDb: clamp(
      ...
      instrumentalLevelDb: clamp(
        safeNumber(
          recombinationPlan.instrumentalLevelDb,
          fallback.recombinationPlan.instrumentalLevelDb
        ),
        -12,
        12
      ),
...
Règles :
- traitement musical et réaliste
- pas d’exagération inutile
- si la voix est déjà brillante, rester prudent
- si l’instrumental est déjà dense, éviter de surcharger
- la voix doit rester lisible
- l’instrumental doit garder de l’espace
- intensité entre 0 et 1
- voiceLevelDb et instrumentalLevelDb entre -12 et 12
- réponds uniquement avec le JSON
`.trim();
