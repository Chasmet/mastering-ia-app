...
async function runPythonSplitChunks(filepath) {
  const scriptPath = path.resolve("split_audio_chunks.py");
  const candidates = ["python3", "python"];
  let lastError = null;
  ...
  for (const cmd of candidates) {
    try {
      ...
      if (!parsed.ok) {
        throw new Error(parsed.error || "Découpage Python invalide");
      }
  ...
  throw new Error(
    `Impossible d'exécuter split_audio_chunks.py : ${lastError?.message || "erreur inconnue"}`
  );
}
...
- enhancement_chain pour enrichir le morceau
- master_chain pour finaliser proprement
...
function buildEnhanceToneFilters(settings = {}) {
  ...
  if (lowGain !== 0) {
    filters.push(`equalizer=f=110:t=q:w=1.2:g=${lowGain}`);
  }

  if (presenceGain !== 0) {
    filters.push(`equalizer=f=2800:t=q:w=1.0:g=${presenceGain}`);
  }

  if (airGain !== 0) {
    filters.push(`equalizer=f=9500:t=q:w=0.7:g=${airGain}`);
  }
...
app.post("/api/split-chunks", upload.single("audio"), async (req, res) => {
  let inputPath = "";
  ...
  try {
    ...
    const splitResult = await runPythonSplitChunks(inputPath);
