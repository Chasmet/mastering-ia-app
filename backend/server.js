...
function getDefaultPlan() {
  return {
    goal: "master propre, puissant et équilibré",
    style: "auto",
    target_lufs: -14,
    true_peak_ceiling_db: -1,
    mastering_intensity: 0.65,
    chain: [
      {
        tool: "cleanup",
        enabled: true,
...
function normalizePlan(plan) {
  ...
  const safeChain = Array.isArray(safePlan.chain) ? safePlan.chain : [];

  return {
    goal: typeof safePlan.goal === "string" ? safePlan.goal : "master propre et équilibré",
    style: typeof safePlan.style === "string" ? safePlan.style : "auto",
    ...
    mastering_intensity: clamp(safeNumber(safePlan.mastering_intensity, 0.65), 0, 1),
    chain: safeChain
...
async function buildPlanWithAI(userPrompt, analysis) {
  const systemPrompt = `
Tu es un ingénieur du son expert en mastering.
...
Règles :
- Utilise seulement les outils autorisés
- Pas d’outil inventé
- Pas de réglages extrêmes
- Reste prudent, musical et propre
- Si le mix semble déjà correct, fais des réglages modérés
- "target_lufs" entre -18 et -7
- "true_peak_ceiling_db" entre -2 et -0.1
- "mastering_intensity" entre 0 et 1
...
function buildEqFilters(settings = {}) {
  ...
  if (lowGain !== 0) {
    filters.push(`equalizer=f=90:t=q:w=1.0:g=${lowGain}`);
  }

  if (presenceGain !== 0) {
    filters.push(`equalizer=f=3500:t=q:w=1.0:g=${presenceGain}`);
  }

  if (airGain !== 0) {
    filters.push(`equalizer=f=10500:t=q:w=0.8:g=${airGain}`);
  }
...
function buildFilterChainFromPlan(plan) {
  ...
  for (const step of chain) {
    ...
    switch (step.tool) {
      case "cleanup": {
        ...
        const lowpassHz = clamp(safeNumber(settings.lowpass_hz, 17500), 12000, 20000);
      ...
      case "highpass": {
        const freqHz = clamp(safeNumber(settings.freq_hz, 28), 20, 120);
        filters.push(`highpass=f=${freqHz}`);
        break;
      }
      ...
      case "compressor": {
        const thresholdDb = clamp(safeNumber(settings.threshold_db, -18), -30, -6);
        const ratio = clamp(safeNumber(settings.ratio, 2.5), 1.1, 6);
        const attackMs = clamp(safeNumber(settings.attack_ms, 20), 1, 200);
        const releaseMs = clamp(safeNumber(settings.release_ms, 220), 20, 1000);
        const makeupGainDb = clamp(safeNumber(settings.makeup_gain_db, 3), 0, 8);
      ...
      case "loudnorm": {
        const targetLufs = clamp(
          safeNumber(settings.target_lufs, plan.target_lufs ?? -14),
          -18,
          -7
        );
        const truePeakDb = clamp(
          safeNumber(settings.true_peak_db, plan.true_peak_ceiling_db ?? -1),
          -2,
          -0.1
        );
  ...
  if (filters.length === 0) {
    ...
    return buildFilterChainFromPlan(fallback);
  }
