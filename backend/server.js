...
function verifyAdminToken(token) {
  ...
  if (signature !== expectedSignature) {
    throw new Error("Signature admin invalide");
  }
...
function sumMoney(rows, key) {
  return Number(
    rows.reduce((sum, row) => sum + safeNumber(row?.[key], 0), 0).toFixed(2)
  );
}

function buildAdminSummary(sales, expenses, quarter, year) {
  const salesTotalEur = sumMoney(sales, "amount_eur");
  const expensesTotalEur = sumMoney(expenses, "amount_eur");
  const declarationTotalEur = salesTotalEur;
  const internalNetEur = Number((salesTotalEur - expensesTotalEur).toFixed(2));

  const monthMap = new Map();

  const sortedSales = [...sales].sort((a, b) => {
    const monthA = safeNumber(a.month_number, 0);
    const monthB = safeNumber(b.month_number, 0);
    return monthA - monthB;
  });

  for (const row of sortedSales) {
    const key = row.month_label || `${row.month_number || ""}/${row.year || ""}`;
    const previous = monthMap.get(key) || 0;
    monthMap.set(key, previous + safeNumber(row.amount_eur, 0));
  }

  const monthlySales = [...monthMap.entries()].map(([monthLabel, total]) => ({
    monthLabel,
...
async function fetchAdminQuarterData(quarterInput, yearInput) {
  const quarter = clamp(safeNumber(quarterInput, 1), 1, 4);
  const year = clamp(safeNumber(yearInput, new Date().getFullYear()), 2020, 2100);
  ...
  const summary = buildAdminSummary(safeSales, safeExpenses, quarter, year);
...
function buildFallbackQuarterReport({ quarter, year, summary, sales, expenses }) {
  const monthlyLines = (summary.monthlySales || [])
    .map((item) => `- ${item.monthLabel} : ${item.totalEur.toFixed(2)} €`)
    .join("\n");

  const expenseLines = (expenses || []).length
    ? expenses
        .map((row) => `- ${row.expense_date} · ${row.category} · ${Number(safeNumber(row.amount_eur, 0)).toFixed(2)} €`)
        .join("\n")
    : "- Aucune dépense enregistrée sur la période.";

  return [
    `Déclaration URSSAF - Trimestre ${quarter} ${year}`,
    "",
    `Période : ${summary.quarterLabel}`,
    "",
    "Chiffre d’affaires encaissé :",
    monthlyLines || "- Aucun encaissement enregistré.",
    "",
    `Total à déclarer : ${summary.declarationTotalEur.toFixed(2)} €`,
    "",
    "Dépenses internes :",
    expenseLines,
    "",
    `Total dépenses internes : ${summary.expensesTotalEur.toFixed(2)} €`,
    `Marge interne indicative : ${summary.internalNetEur.toFixed(2)} €`,
    "",
    "Note :",
    "Le montant principal à déclarer au régime micro-entrepreneur correspond ici au chiffre d’affaires encaissé.",
    "Les dépenses internes servent au pilotage de l’activité."
  ].join("\n");
}

async function buildAdminQuarterReportWithAI(context) {
  const systemPrompt = `
Tu es l'assistant comptable interne d'une micro-entreprise française.
Tu rédiges un rapport trimestriel en français clair.
Tu distingues toujours :
- le chiffre d'affaires encaissé à déclarer
- les dépenses internes de pilotage
Tu restes concret, utile, simple.
Aucun jargon inutile.
`.trim();

  const userPrompt = `
Période :
Trimestre ${context.quarter} ${context.year}
...
Consignes :
- fais un rapport propre et facile à copier
- indique clairement le total à déclarer
- ajoute un petit commentaire pratique de pilotage
`.trim();
...
app.get("/api/admin/dashboard", requireAdmin, async (req, res) => {
  try {
    ...
    const data = await fetchAdminQuarterData(quarter, year);
...
app.post("/api/admin/report", requireAdmin, async (req, res) => {
  try {
    ...
    const context = await fetchAdminQuarterData(quarter, year);

    let reportText = buildFallbackQuarterReport(context);

    try {
      const aiReport = await buildAdminQuarterReportWithAI(context);
      if (aiReport) {
        reportText = aiReport;
      }
    } catch (_) {}
    ...
    return res.status(500).json({
      ok: false,
      error: error.message || "Rapport trimestriel impossible"
    });
  }
});
