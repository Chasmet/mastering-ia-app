...
function buildEnhanceToneFilters(settings = {}) {
  ...
  const presenceGain = clamp(safeNumber(settings.presence_gain_db, 0.7), -4, 4);
  const airGain = clamp(safeNumber(settings.air_gain_db, 0.3), -4, 4);
...
function buildEnhanceGlueFilters(settings = {}) {
  ...
  const makeupGainDb = clamp(safeNumber(settings.makeup_gain_db, 1.0), 1, 8);
  ...
  for (const step of chain) {
    ...
    switch (step.tool) {
      ...
      case "cleanup": {
        ...
        const lowpassHz = clamp(safeNumber(settings.lowpass_hz, 18000), 10000, 20000);
...
function buildVoiceStemFilters(stemPlan) {
  ...
  const intensity = clamp(safeNumber(stemPlan.voicePlan.intensity, 0.5), 0, 1);

  const bodyGain = clamp(0.4 + intensity * 0.8, 0, 1.4);
  const presenceGain = clamp(s.presence_gain_db, -3, 3);
  const deHarshGain = clamp(-0.35 - intensity * 0.55, -1.2, 0);
  const airGain = clamp(s.air_gain_db, -2, 2);

  const filters = ["highpass=f=70"];

  if (bodyGain !== 0) {
    filters.push(`equalizer=f=180:t=q:w=1.1:g=${bodyGain.toFixed(2)}`);
  }
  if (presenceGain !== 0) {
    filters.push(`equalizer=f=3200:t=q:w=1.0:g=${presenceGain}`);
  }
  if (deHarshGain !== 0) {
    filters.push(`equalizer=f=6200:t=q:w=1.1:g=${deHarshGain.toFixed(2)}`);
  }
  if (airGain !== 0) {
    filters.push(`equalizer=f=9800:t=q:w=0.8:g=${airGain}`);
  }
  ...
  filters.push("alimiter=limit=0.9700");
...
function buildInstrumentalStemFilters(stemPlan) {
  ...
  const intensity = clamp(safeNumber(stemPlan.instrumentalPlan.intensity, 0.5), 0, 1);

  const lowGain = clamp(s.low_gain_db, -3, 3);
  const bodyGlueGain = clamp(0.2 + intensity * 0.4, 0, 0.7);
  const presenceGain = clamp(s.presence_gain_db, -3, 3);
  const topTameGain = clamp(-0.2 - intensity * 0.4, -0.9, 0);

  const filters = ["highpass=f=28"];

  if (lowGain !== 0) {
    filters.push(`equalizer=f=95:t=q:w=1.1:g=${lowGain}`);
  }
  if (bodyGlueGain !== 0) {
    filters.push(`equalizer=f=220:t=q:w=1.0:g=${bodyGlueGain.toFixed(2)}`);
  }
  if (presenceGain !== 0) {
    filters.push(`equalizer=f=3200:t=q:w=1.0:g=${presenceGain}`);
  }
  if (topTameGain !== 0) {
    filters.push(`equalizer=f=8500:t=q:w=0.9:g=${topTameGain.toFixed(2)}`);
  }

  filters.push(
    `acompressor=threshold=-19dB:ratio=${s.glue_ratio}:attack=${s.glue_attack_ms}:release=${s.glue_release_ms}:makeup=1`
  );
  filters.push("alimiter=limit=0.9800");
...
app.post("/api/master-stems", upload.fields([
  ...
  try {
    ...
    await runFfmpeg([
      ...
      instrumentalProcessedPath,
      "-filter_complex",
      `[0:a]volume=${voiceGain.toFixed(4)},alimiter=limit=0.9800[v];[1:a]volume=${instrumentalGain.toFixed(4)},alimiter=limit=0.9800[m];[v][m]amix=inputs=2:normalize=0,highpass=f=24,equalizer=f=110:t=q:w=1.0:g=0.25,equalizer=f=3200:t=q:w=1.0:g=-0.15,alimiter=limit=0.8913,loudnorm=I=-14:TP=-1:LRA=11[aout]`,
      "-map",
      "[aout]",
      "-ar",
      "44100",
