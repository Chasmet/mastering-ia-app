...
import {
  ...
  createExpenseV2,
  buildQuarterDataV2,
  buildQuarterReportWithAIV2
} from "./accounting-v2.js";
...
app.get("/api/admin/dashboard", requireAdmin, async (req, res) => {
  try {
    ...
    const context = await buildQuarterDataV2({
      supabase,
      quarterInput: quarter,
      yearInput: year,
      getCurrentQuarterYear,
      clamp,
      safeNumber,
    ...
    const reportText = await buildQuarterReportWithAIV2({
      openai,
      context,
      formatEuroText
    });
...
app.post("/api/admin/expenses", requireAdmin, async (req, res) => {
  try {
    const data = await createExpenseV2({
      supabase,
      payload: req.body || {},
      normalizeDateInput,
      buildPeriodInfo,
