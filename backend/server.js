...
async function buildInternalAssistantReply(payload) {
  const fallback = {
    shortSummary: "Demande analysée.",
    priority: "normale",
    adminAction: "Vérifier la demande puis répondre au client.",
    statusSuggestion: "a_traiter",
    suggestedReply: "Bonjour, votre demande a bien été prise en compte. Nous vérifions la situation et revenons vers vous rapidement."
  };

  const systemPrompt = `
Tu es l'assistant interne privé de Mastering IA.
Tu aides uniquement l'administrateur.
Tu ne parles jamais comme si tu étais visible par le client.
Tu réponds uniquement avec un JSON valide.
...
- ton rôle est d'aider l'admin à répondre vite et proprement
`.trim();
  ...
  try {
    ...
    const parsed = extractJson(response.output_text || "");
    return {
      shortSummary: normalizeText(parsed.shortSummary, fallback.shortSummary),
      priority: normalizeText(parsed.priority, fallback.priority),
      adminAction: normalizeText(parsed.adminAction, fallback.adminAction),
      statusSuggestion: normalizeText(parsed.statusSuggestion, fallback.statusSuggestion),
      suggestedReply: normalizeText(parsed.suggestedReply, fallback.suggestedReply)
    };
  } catch {
    return fallback;
  }
...
async function listClientPaymentRequests(email) {
  const { data, error } = await supabase
    .from("payment_requests")
    .select("*")
...
async function listAdminPaymentRequests() {
  const { data, error } = await supabase
    .from("payment_requests")
    .select("*")
    .order("created_at", { ascending: false })
...
async function createPaymentRequest({ clientUser, packKey, clientNote }) {
  const pack = getPackConfig(packKey);

  if (!pack) {
    throw buildHttpError(400, "Pack invalide");
  }
  ...
  const { data, error } = await supabase
    ...
    .insert([{
      ...
      client_note: normalizeText(clientNote, ""),
      status: "en_attente"
    }])
    .select()
    .single();
...
async function approvePaymentRequest({ paymentRequestId, adminEmail, adminNote }) {
  const paymentRequest = await getPaymentRequestById(paymentRequestId);

  if (!paymentRequest) {
    throw buildHttpError(404, "Demande de paiement introuvable");
  }

  if (paymentRequest.status !== "en_attente") {
    throw buildHttpError(409, "Cette demande a déjà été traitée");
  }

  const targetEmail = normalizeEmail(paymentRequest.client_email);
  const user = paymentRequest.client_user_id
    ? await getUserById(paymentRequest.client_user_id)
    : await getUserByEmail(targetEmail);

  if (!user) {
    throw buildHttpError(404, "Client introuvable pour cette demande");
  }

  const nextBalance = Math.max(
    0,
    safeNumber(user.current_generations, 0) + Math.max(0, safeNumber(paymentRequest.generations_to_add, 0))
  );
  ...
  await insertGenerationEvent({
    userId: user.id,
    delta: Math.max(0, safeNumber(paymentRequest.generations_to_add, 0)),
    reason: "payment_request_approved",
    note: `Recharge manuelle validée - ${paymentRequest.pack_name}`
  });

  const saleDate = normalizeDateInput(new Date());
  ...
  const salePayload = {
    ...
    amount_eur: Number(safeNumber(paymentRequest.amount_eur, 0).toFixed(2)),
    generations_added: Math.max(0, safeNumber(paymentRequest.generations_to_add, 0)),
    payment_method: "Revolut",
    note: normalizeText(adminNote, "") || `Validation manuelle demande ${paymentRequest.id}`
  };
  ...
  const { data: updatedPayment, error: paymentError } = await supabase
    ...
    .update({
      ...
      treated_by: adminEmail,
      admin_note: normalizeText(adminNote, ""),
...
async function cancelPaymentRequest({ paymentRequestId, adminEmail, adminNote }) {
  const paymentRequest = await getPaymentRequestById(paymentRequestId);
...
app.get("/api/client/payment-requests", requireClient, async (req, res) => {
  try {
    const rows = await listClientPaymentRequests(req.client.email);
...
app.get("/api/admin/payment-requests", requireAdmin, async (req, res) => {
  try {
    const rows = await listAdminPaymentRequests();
    return res.json({
      ok: true,
...
app.post("/api/admin/payment-requests/:id/cancel", requireAdmin, async (req, res) => {
  try {
    const paymentRequest = await cancelPaymentRequest({
      paymentRequestId: req.params.id,
      adminEmail: req.admin.email,
      adminNote: req.body?.adminNote
    });
...
app.post("/api/admin/internal-assistant", requireAdmin, async (req, res) => {
  try {
    const result = await buildInternalAssistantReply({
