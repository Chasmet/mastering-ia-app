...
app.post("/api/master-stems", upload.fields([
  ...
  try {
    ...
    if (!adminBypass) {
      const clientPayload = verifySignedToken(CLIENT_SECRET, readBearerToken(req), "client");
      clientUser = await getUserById(clientPayload.userId);
      ...
      if (safeNumber(clientUser.current_generations, 0) < 1) {
        return res.status(403).json({
          ok: false,
          error: "Tu n’as plus assez de générations"
        });
      }
    }
    ...
    if (!adminBypass && clientUser) {
      const nextBalance = Math.max(0, safeNumber(clientUser.current_generations, 0) - 1);
      await updateUserGenerations(clientUser.id, nextBalance);
      await insertGenerationEvent({
        userId: clientUser.id,
        delta: -1,
        reason: "final_master",
        note: "1 génération utilisée pour un traitement final"
      });
