...
def main():
    ...
    try:
        ...
        total_duration_sec = info.frames / info.samplerate

        frames_to_read = min(info.frames, int(MAX_SECONDS_TO_ANALYZE * info.samplerate))
        data, sr = sf.read(filepath, frames=frames_to_read, always_2d=True, dtype="float32")
        ...
        analyzed_duration_sec = len(mono) / sr
        ...
        high_energy = np.sum(spectrum[(freqs >= 4000) & (freqs < 11000)])
        ...
        result = {
            ...
            "analysis": {
                ...
                "analyzedDurationSec": safe_float(analyzed_duration_sec, 2),
                "sampleRate": int(sr),
