...
def frame_rms_db_values(signal, frame_size=FRAME_SIZE, hop=HOP_SIZE):
    values = []
...
def spectral_metrics(signal, sr):
    ...
    for start in range(0, len(signal) - FRAME_SIZE + 1, HOP_SIZE):
        ...
        high_energy = float(np.sum(spectrum[(freqs >= 4000) & (freqs <= nyquist)]))
    ...
    return {
        "centroid": float(np.mean(frame_centroids)),
        "rolloff": float(np.mean(frame_rolloffs)),
        "flatness": float(np.mean(frame_flatness)),
        "low_energy": total_low_energy,
        "mid_energy": total_mid_energy,
        "high_energy": total_high_energy,
        "total_energy": max(total_energy_sum, 1e-12)
    }
    ...
    try:
        ...
        with sf.SoundFile(filepath, "r") as f:
            while True:
                ...
                centroid_values.append(metrics["centroid"])
                rolloff_values.append(metrics["rolloff"])
                flatness_values.append(metrics["flatness"])
        ...
        if dynamic_rms_values:
            p95 = float(np.percentile(dynamic_rms_values, 95))
            p10 = float(np.percentile(dynamic_rms_values, 10))
            dynamic_range_db = max(0.0, p95 - p10)
            dynamic_range_db = min(dynamic_range_db, 24.0)
        else:
            dynamic_range_db = max(0.0, min(24.0, true_peak_db - rms_db))

        centroid = float(np.mean(centroid_values)) if centroid_values else 0.0
        rolloff = float(np.mean(rolloff_values)) if rolloff_values else 0.0
        flatness = float(np.mean(flatness_values)) if flatness_values else 0.0

        analysis_sr = TARGET_SR if orig_sr != TARGET_SR else orig_sr
        nyquist = analysis_sr / 2.0
        rolloff = min(max(rolloff, 0.0), nyquist)
        ...
        ratio_sum = low_ratio + mid_ratio + high_ratio
        if ratio_sum > 0:
            low_ratio /= ratio_sum
            mid_ratio /= ratio_sum
            high_ratio /= ratio_sum
