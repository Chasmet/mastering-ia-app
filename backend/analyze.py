...
def spectral_metrics(signal, sr):
    ...
    high_energy = float(np.sum(spectrum[(freqs >= 4000) & (freqs <= sr / 2)]))
    ...
    try:
        ...
        with sf.SoundFile(filepath, "r") as f:
            while True:
                ...
                centroid_weighted_sum += metrics["centroid"] * weight
                rolloff_weighted_sum += metrics["rolloff"] * weight
                flatness_weighted_sum += metrics["flatness"] * weight
                weight_sum += weight
        ...
        centroid = centroid_weighted_sum / max(weight_sum, 1)
        rolloff = rolloff_weighted_sum / max(weight_sum, 1)
        flatness = flatness_weighted_sum / max(weight_sum, 1)
